...
function createMessageDiv(messageId, message) {
  ...
  if (message.uid === currentUID) { // 送信メッセージ
    $div = $(".message-template .message--sent").clone(true);
  } else { // 受信メッセージ
    $div = $(".message-template .message--received").clone(true);
  }
...
function _showRoom(roomName) {
  ...
  var roomRef = firebase.database().ref("messages/" + roomName).limitToLast(5) ;
  // 過去に登録したイベントハンドラを削除
...
function deleteRoom(roomName) {
  ...
  // room1を削除
  firebase.database().ref("rooms/room1").remove();

  // room1内のメッセージも削除
  firebase.database().ref("messages/room1").remove();
...
//*********************お気に入りのデータベース処理 *************************
function addToFavorites(msgID){
  //var user = dbdata.users[message.uid]; //sample
  var msg = null;
  var messageRef = firebase.database().ref("messages").child(currentRoomName).child(msgID);
  console.log("everything " + messageRef);
  messageRef.once("value", function(snapshot) {
    msg = snapshot.val();
  });

  favoriteRef = firebase.database().ref().child("favorites/" + currentUID + "/" + msgID);
...
} //endof addToFavorites


//*******************お気に入りを表示する*****************************************
function loadFavoritesView(){
  var favoriteMessagesRef = firebase.database().ref("favorites");
} //endof loadFavoritesView
...
$(document).ready(function() {
  ...
  $(".favorite").click(function(){
    var favoriteMessageID = $(this).parent().parent().attr("id").substring(11);
    console.log(favoriteMessageID);
    //お気に入りに登録されていない場合は、お気に入りに追加する
    if($(this).children().hasClass("glyphicon-star-empty")){
      //addとremoveでやったほうがいい
      $(this).html("<span class='glyphicon glyphicon-star'></span>");
      addToFavorites(favoriteMessageID);
    } else {
    //お気に入りに登録されている場合は、お気に入りから外す
